...
class URLDecoder {
    ...
    func decode() {
        ...
        if let queryItems = components?.queryItems {
            ...
            var uCards = [Int]()
            var uCardsPlayer : UUID? = nil //just to get around safety warnings for use before initialization
            ...
            for item in queryItems {
                ...
                if mode.rawValue == 0 { //used for changing which type of object we are decoding info for or general simple info
                    ...
                    case "id" :
                        players[players.count - 1].id = UUID(uuidString: item.value!)!
                    case "hand" :
                    ...
                    case "card_player":
                        ...
                        if let playerStr = item.value {
                            uCardsPlayer = UUID(uuidString: playerStr)
                        }
                    case "usedCardEnd":
